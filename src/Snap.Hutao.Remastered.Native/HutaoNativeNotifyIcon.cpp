...
LRESULT CALLBACK HutaoNativeNotifyIcon::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (pThis != nullptr)
	{
		if (message == GetTaskbarCreatedMessage() || message == pThis->m_uCallbackMessage)
		{
...
HRESULT __stdcall HutaoNativeNotifyIcon::Recreate(PCWSTR tip)
{
	...
	{
		// Explorer may have already dropped the icon during a restart.
		// Re-adding is still the important step here.
	}
